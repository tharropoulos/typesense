...
bool Collection::handle_highlight_text(std::string& text, const bool& normalise, const field& search_field,
    ...
    if (is_phrase_query && is_arr_obj_ele && match.offsets.empty() && !text.empty()) {
        ...
        // check if any phrase from q_phrases appears consecutively in the text
        ...
        for(const auto& phrase : q_phrases) {
            ...
            if(phrase.size() > text_tokens.size()) {
                continue;
            }
            
            for(size_t i = 0; i <= text_tokens.size() - phrase.size(); i++) {
                bool phrase_matches = true;
                for(size_t j = 0; j < phrase.size(); j++) {
                    std::string text_token_lower = text_tokens[i + j];
                    std::string phrase_token_lower = phrase[j];
                    StringUtils::tolowercase(text_token_lower);
                    StringUtils::tolowercase(phrase_token_lower);
                    
                    if(text_token_lower != phrase_token_lower) {
                ...
                if(phrase_matches) {
                    ...
                    break; // found a match for this phrase
                }
            }
        }
        ...
        size_t snippet_end_offset = (text.empty() ? 0 : text.size() - 1);
        ...
        if(text_len <= snippet_threshold-1) {
            snippet_start_offset = 0;
